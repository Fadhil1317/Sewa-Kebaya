...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/products`) // Pakai template literal (tanda backtick ` `)
      .then((res) => res.json())
      .then((data) => {
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FDFCF8]">
        <div className="animate-pulse font-serif italic text-amber-900">
          Menyampirkan Kain...
        </div>
      </div>
    );
  ...
  if (!product)
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#FDFCF8]">
        ...
        <button
          onClick={() => navigate("/")}
          className="mt-4 px-8 py-3 bg-stone-900 text-white rounded-full uppercase text-xs tracking-widest font-bold"
        >
          Kembali Ke Galeri
        </button>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 md:py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-start">
          ...
          <div className="relative group">
            <div
              className={`aspect-4/5 bg-white rounded-[2.5rem] border border-stone-200 flex items-center justify-center p-8 shadow-2xl shadow-stone-200/50 overflow-hidden transition-all duration-700 ${!isAvailable ? "grayscale opacity-60" : ""}`}
            >
          ...
          <div className="flex flex-col">
            ...
            <h1
              className={`text-4xl md:text-5xl font-serif font-medium mb-4 leading-tight ${!isAvailable ? "text-stone-400" : "text-stone-900"}`}
            >
              {product.name}
            </h1>

            <p
              className={`text-3xl font-light mb-8 pb-8 border-b border-stone-200/60 ${!isAvailable ? "text-stone-300" : "text-amber-900"}`}
            >
              <span className="text-sm align-top mr-1 uppercase text-stone-400 font-sans">
                IDR
              </span>
            ...
            <div className="space-y-8 mb-10">
              <div>
                ...
                <p className="text-stone-700 leading-relaxed text-lg font-serif italic">
                  "
                  {product.description ||
                    "Koleksi pilihan dengan kualitas jahitan premium, dirancang khusus untuk kenyamanan dan keanggunan pemakainya."}
                  "
                </p>
              </div>
                ...
                <div className="bg-stone-50 p-6 rounded-3xl border border-stone-100 shadow-inner">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center gap-4 bg-white border border-stone-200 rounded-2xl p-2 shadow-sm">
                      ...
                      <span className="text-base font-bold w-12 text-center text-stone-800">
                        {duration} Hari
                      </span>
                      <button
